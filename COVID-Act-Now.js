...
async function getCovidData(fipsCode) {
  const fipsCache = `fips-${fipsCode}-summary-data.json`;
  const req = new Request(`${COVID_API_URL}/county/${fipsCode}.json?apiKey=${COVID_API_KEY}`);
  let json = await req.loadJSON();

  try {
    // Check that our results are what we expect
    if (json && json.fips && json.riskLevels && json.riskLevels.overall && json.fips && json.county && json.state && json.lastUpdatedDate && json.metrics && json.metrics.caseDensity && json.url) {
      console.log(`COVID summary data looks good, will cache.`);
      ...
      console.log(`Using cached COVID summary data: ${updatedAt}`);
      json = cachedJson;
    }
    return {
      fips: json.fips,
...
async function run() {
  ...
  try {
    ...
    const trendImg = scoreStack.addImage(riskSymbol.image);
